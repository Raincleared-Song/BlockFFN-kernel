...
template <typename T, typename T2>
__global__ void dot_product_kernel(int dim, const T2* input, const T2* weight, T* output) {
    __shared__ float shared_sum;
    __shared__ float warp_sum[16];
    int row = blockIdx.x;
    int col = threadIdx.x;
    ...
    for (int i = col; i < dim; i += blockDim.x) {
        T2 val = input[row * dim + i];
        T2 w = weight[i];
        float val1 = float(val.x) * float(w.x);
        float val2 = float(val.y) * float(w.y);
        sum1 += val1;
        sum2 += val2;
    }
    float sum = sum1 + sum2;
...
template <typename T>
__global__ void minus_mean_kernel(int num_tokens, int dim, T* input, T* mean, T* output) {
    int row = blockIdx.x;
    int col = threadIdx.x;
    T m = mean[blockIdx.x];
    
    for (int i = col; i < dim; i += blockDim.x) {
        output[row * dim + i] = input[row * dim + i] - m;
    }
}
...
template <typename T>
void minus_mean(const Stream& stream, int num_tokens, int dim, T* input, T* mean, T* output) {
    minus_mean_kernel<T><<<num_tokens, 1024, 0, stream.stream>>>(num_tokens, dim, input, mean, output);
}

template <typename T>
void dot_product(const Stream& stream, int num_tokens, int dim, T* input, T* mean, T* output) {
    using T2 = typename TypeTraits<T>::half2;
    dot_product_kernel<T, T2><<<num_tokens, 512, 0, stream.stream>>>(dim/2, (T2*)input, (T2*)mean, output);
}
}
...
struct SimpleLayerNorm {
    ...
    void prefill(const Stream& stream, int32_t num_tokens, T* input, T* tgt=nullptr) {
        if (tgt == nullptr) tgt = this->output;
        elementwise_mul(stream, num_tokens, this->dim, input, this->weight, tgt);
    }
};
...
struct NormSiLU {
    ...
    NormSiLU(int num_blocks, int block_size, float eps) {
    ...
    void init_weight_ptr(Memory* memory) {
        weight = (T*)memory->allocate_for_model(block_size * sizeof(T));
    }

    int64_t init_output_ptr(Memory* memory, int32_t num_tokens, int64_t offset) {
        return memory->allocate((void**)&this->output, offset, num_tokens * num_blocks * block_size * sizeof(T));
    }

    void load_to_storage(std::string name, void* ptr) {
        cudaMemcpy((void*)weight, ptr, block_size * sizeof(T), cudaMemcpyHostToDevice);
    }

    void prefill(const Stream& stream, int32_t num_tokens, T* input, T* projected_mean, T* tgt=nullptr) {
        if (tgt == nullptr) tgt = this->output;
        minus_mean(stream, num_tokens, this->num_blocks * this->block_size, input, projected_mean, tgt);
        rms_norm(stream, num_tokens * this->num_blocks, this->block_size, tgt, this->weight, tgt, this->eps);
        silu_inplace(stream, num_tokens, this->num_blocks * this->block_size, tgt);
    }
};