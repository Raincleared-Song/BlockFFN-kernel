...
class LLM(torch.nn.Module):
    def __init__(self,
        ...
        if not hasattr(self.config, "head_dim"):
            if hasattr(self.config, "attn_type") and self.config.attn_type == "mla":
    ...
    def _load(self, name, param, dtype=None, cls=None):
        ...
        if 'gate_up_proj' in name:
            ...
            if name.endswith("proj"): name = name + ".weight"
            if 'expert_up_proj' in name:
                m = param.mean(dim=0)
                C.load_model(name.replace("expert_up_proj", "expert_up_proj_mean"), m.data_ptr())
